...
#data.path <- "/scratch/data/footprints/lymphatic_vessel_wellington_16"
data.path <- "/scratch/shared/footprints/lymphatic_vessel_wellington_16"
...
if(!exists("db.wellington"))
    db.wellington <- "lymphatic_vessel_wellington_16"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "lymphatic_vessel_wellington_16.minid",
             dbUser = "trena",
             dbTable = "lymphatic_vessel_wellington_16",
