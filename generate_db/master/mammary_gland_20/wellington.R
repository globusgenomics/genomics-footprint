...
#data.path <- "/scratch/data/footprints/mammary_gland_wellington_20"
data.path <- "/scratch/shared/footprints/mammary_gland_wellington_20"
output_path=paste(data.path,"/TFBS_OUTPUT",sep="")
dir.create(output_path, showWarnings = FALSE)

bdbag.path<-"/scratch/shared/footprints/mammary_gland_20"
dir.create(bdbag.path, showWarnings = FALSE)
#-------------------------------------------------------------------------------
# establish database connections:

if(!exists("db.wellington"))
    db.wellington <- "mammary_gland_wellington_20"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "mammary_gland_wellington_20.minid",
             dbUser = "trena",
             dbTable = "mammary_gland_wellington_20",
