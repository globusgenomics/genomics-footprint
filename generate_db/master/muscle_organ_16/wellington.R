...
#data.path <- "/scratch/data/footprints/muscle_organ_wellington_16"
data.path <- "/scratch/shared/footprints/muscle_organ_wellington_16"
...
if(!exists("db.wellington"))
    db.wellington <- "muscle_organ_wellington_16"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "muscle_organ_wellington_16.minid",
             dbUser = "trena",
             dbTable = "muscle_organ_wellington_16",
