...
#data.path <- "/scratch/data/footprints/prostate_gland_wellington_16"
data.path <- "/scratch/shared/footprints/prostate_gland_wellington_16"
...
if(!exists("db.wellington"))
    db.wellington <- "prostate_gland_wellington_16"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "prostate_gland_wellington_16.minid",
             dbUser = "trena",
             dbTable = "prostate_gland_wellington_16",
