...
#data.path <- "/scratch/data/footprints/prostate_gland_wellington_20"
data.path <- "/scratch/shared/footprints/prostate_gland_wellington_20"
...
if(!exists("db.wellington"))
    db.wellington <- "prostate_gland_wellington_20"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "prostate_gland_wellington_20.minid",
             dbUser = "trena",
             dbTable = "prostate_gland_wellington_20",
