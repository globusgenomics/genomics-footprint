...
#data.path <- "/scratch/data/footprints/thyroid_gland_wellington_16"
data.path <- "/scratch/shared/footprints/thyroid_gland_wellington_16"
...
if(!exists("db.wellington"))
    db.wellington <- "thyroid_gland_wellington_16"
...
if(!interactive()){
    ...
    result <- bptry(bplapply(chromosomes, fillAllSamplesByChromosome,
             ...
             fimo = db.fimo,
             minid = "thyroid_gland_wellington_16.minid",
             dbUser = "trena",
             dbTable = "thyroid_gland_wellington_16",
